...
async def new_browser(botright: Botright, proxy: ProxyManager, faker: Faker, flags: List[str], **launch_arguments) -> BrowserContext:
    ...
    """
    # Extract per-call overrides without polluting kwargs passed to Playwright
    extra_args = launch_arguments.pop("extra_args", None)
    viewport_override_provided = "viewport" in launch_arguments
    viewport_override = launch_arguments.pop("viewport", None)
    ...
    if viewport_override_provided:
        parsed_launch_arguments["viewport"] = viewport_override
        if viewport_override:
            parsed_launch_arguments["screen"] = {
                "width": viewport_override.get("width"),
                "height": viewport_override.get("height"),
            }
    ...
    combined_args = flags + (extra_args or [])
    if proxy.browser_proxy:
        _browser = await botright.playwright.chromium.launch_persistent_context(
            user_data_dir=temp_dir_path, headless=botright.headless, executable_path=botright.browser["path"], args=combined_args, chromium_sandbox=True, **parsed_launch_arguments
        )
    else:
        _browser = await botright.playwright.chromium.launch_persistent_context(
            user_data_dir=temp_dir_path, headless=botright.headless, executable_path=botright.browser["path"], args=combined_args, chromium_sandbox=True, **parsed_launch_arguments
