...
# Thêm vào cuối file page.py, trong class Page hoặc dưới các hàm custom thuộc class này

    async def bypass_press_hold_challenge(self, selector="div[aria-label='Press & Hold Human Challenge']", hold_time=3):
        btn = self.locator(selector)
        await btn.hover()
        ...
        await asyncio.sleep(hold_time)   # thời gian giữ chuột (ms hoặc s)
        await self.mouse.up()
