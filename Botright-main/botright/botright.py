...
class Botright(AsyncObject):
    ...
    def delete_botright_temp_dirs() -> None:
        ...
        for temp_dir in os.listdir(temp_path):
            ...
            if os.path.isdir(os.path.join(temp_path, temp_dir)) and temp_dir.startswith("botright-"):
                # If it matches, attempt to delete the folder and its contents.
                # On Windows, Chromium profile files may still be locked by another process.
                # In that case, skip instead of crashing.
                target = os.path.join(temp_path, temp_dir)
                try:
                    shutil.rmtree(target, onerror=Botright._handle_remove_readonly)
                except PermissionError:
                    # Skip locked directories (e.g., file in use: WinError 32)
                    pass
                except OSError as e:
                    if getattr(e, "winerror", None) == 32:
                        # File in use by another process; skip
                        pass
                    else:
    ...
    def _handle_remove_readonly(func, path, exc_info):
        # Make read-only files writable, then retry deletion
        try:
            os.chmod(path, stat.S_IWUSR)
            func(path)
        except Exception:
            # Ignore if still failing; higher-level handler will decide
            pass
